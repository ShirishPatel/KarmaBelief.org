...
(function ($) {
  ...
  document.querySelectorAll('.short-text').forEach(function (pTag) {
    const fullText = pTag.getAttribute('data-fulltext');
    const words = fullText.split(" ");
    const shortText = words.slice(0, 25).join(" ") + "...";
    pTag.innerText = shortText;

    const readMore = pTag.nextElementSibling;
    readMore.addEventListener('click', function (e) {
      e.preventDefault();
      if (pTag.innerText === shortText) {
        pTag.innerText = fullText;
        readMore.innerHTML = 'Read Less <i class="fa fa-chevron-double-up"></i>';
      } else {
        pTag.innerText = shortText;
        readMore.innerHTML = 'Read More <i class="fa fa-chevron-double-right"></i>';
      }
    });
  });
